...
import type { UpdateConditionalPrice } from "@routes/locations/common/schema";
import { UpdateConditionalPriceSchema } from "@routes/locations/common/schema";
...
export function EditShippingOptionsPricingForm({
  ...
  const { mutateAsync, isPending } = useUpdateShippingOptions(
    shippingOption.id,
  );
...
const findRuleValue = (
  ...
  const fallbackValue = ["eq", "gt", "lt"].includes(operator)
    ? undefined
    : null;
